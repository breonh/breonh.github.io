...
date: "April 28, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
* If you need to set a working directory, put `setwd()` at the start of your file so that someone using another computer knows they need to modify it
...
If you're interested in advanced project management, email me after class for more resources. 
...
```
![](img/screenshot_relative_path.png)
```
...
10-08-2021,"Pheonix Mercury",87,"Las Vegas Aces",84,Box Score
10-10-2021,"Chicago Sky",91,"Pheonix Mercury",77,Box Score
10-13-2021,"Chicago Sky",86,"Pheonix Mercury",91,Box Score
10-15-2021,"Pheonix Mercury",50,"Chicago Sky",86,Box Score
10-17-2021,"Pheonix Mercury",74,"Chicago Sky",80,Box Score
```
...
.footnote[[1] [`vroom`](https://www.tidyverse.org/blog/2019/05/vroom-1-0-0/#speed) is even faster!]
...
.smallish[
* Did the [last rows/columns](http://unionstats.com/wage_diff_All_Wage_and_Salary.xlsx) from the original file make it in?
...
* Are there ["decorative"](https://www150.statcan.gc.ca/t1/tbl1/#?pid=13100114&file=1310011401-eng.csv) blank rows or columns to remove?
...
* **`separate()`**: pulls apart one column into multiple columns (common after `pivot_longer()` where values are embedded in column names)
    * `extract_numeric()` does a simple version of this for the common case when you just want grab the number part. It is deprecated, however.
...
I will be out of town and sparsely available for email and slack. 

The recordings for lab that would have taken place on May 3rd will be posted on May 3rd. 

There will be no office hours on May 4th. 

The recording for lecture that would have taken place on May 5th will be posted on May 5th.  
