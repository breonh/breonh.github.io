...
date: "May 19, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
`sep=` controls what happens when doing entry-wise squeezing of vectors you give to `paste()`, while `collapse=` controls if/how they go from a vector to a single string.
...
`str_to_sentence` converts cases for a first character capital letter, which may be a good idea for modular reports, but beware the coercion.

```{r make_seattle_sentences}
elegy <- restaurants %>%
...
to a string input, and then compute with **matches** we find.
