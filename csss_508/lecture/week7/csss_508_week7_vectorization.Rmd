...
date: "May 12, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
* `dplyr::filter()`:
    + Input: a data frame, logical condition(s)
...
*Nearly anything you can do with an explicit loop can be done more easily with the `apply` family of functions<sup>1</sup>*

.footnote[
[1] The `tidyverse` version of `apply` functions is called [`purrr`](https://purrr.tidyverse.org/) and has many more options for methods of iteration than base R. 
]
