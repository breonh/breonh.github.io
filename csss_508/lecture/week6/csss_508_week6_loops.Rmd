...
date: "May 5, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
`for()` loops are the most general kind of *loop*, found in pretty much every programming language.<sup>1</sup>
...
.footnote[[1] As early on as [assembler](https://youtu.be/HXNhEYqFo0o), over 70 years ago!]
---
# `for()` Loop: Diagram

*Given a set of values...*

![](https://breonh.github.io/csss_508/lecture/week6/images/loop.svg)
...
data.frame("iters" = 1:iters, "output" = output) %>%   
  ...
  geom_point(size = 2, color = "tomato2") +
  xlab("Iterations") +
  ylab("Calculations") +
...
Reminder: HW 5 Part II assigned last week is due midnight Wednesday.
