...
data.frame("iters" = 1:iters, "output" = output) %>%   
  ggplot(aes(x = iters, y = output)) +
  scale_x_continuous(breaks = 1:10) +
  geom_point() +
  xlab("Iterations") +
  ylab("Calculations") +
  ggtitle("Example of The Value of Assignment for Loops")
  theme_bw(14)
...
## ---- cache=FALSE----------------------------------------------------------------------
set.seed(755) # Making sure values always the same
n <- 300
...
tidy_predicted_data <- predicted_data %>%
    pivot_longer(3:length(predicted_data), 
                 names_to  = "Model", 
