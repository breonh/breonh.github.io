...
pb_state <- 
  get_acs(geography = "tract", state = "GA",
          geometry  = TRUE,
          variables = c("B02001_001E", 
                        "B02009_001E"),
          output = "wide") %>%
...
urbans <- tigris::urban_areas(class = "sf")
glimpse(urbans)

glimpse(urbans)

urban_ga <- urbans %>% filter(str_detect(NAME10, "GA"))
...
pb_state %>% 
  ggplot(aes(fill=`Any Black`)) + 
  geom_sf(size = NA) + 
  geom_sf(data = urban_ga, color = "black", fill = NA, size = 0.1, inherit.aes=FALSE) +
  coord_sf(crs = "+proj=longlat +datum=WGS84", datum=NA) + 
  scale_fill_continuous(name = "Any Black\n", 
                        low  = "#d4d5f9",
                        high = "#00025b") + 
