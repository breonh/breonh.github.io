...
date: "April 14, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
`object %>% function(argument = y)` is the same as 

`function(object, argument = y)`. 

--

Read aloud the ` %>%` is "and then". 
...
---
## Sampling Rows: `slice_sample`

We can also filter *at random* to work with a smaller dataset using `slice_sample()`.
...
`summarize()`<sup>1</sup> takes your column(s) of data and computes something using every row: 
...
.footnote[[1] Note: Can also be written `summarise()`]
...
Maybe you need to calculate the mean and standard deviation of a bunch of columns. With `across()`, put the variables to compute (using `c()` or `select()` syntax) and put the functions to use in a `list()` after.
...
RStudio also helps you by tracking parentheses: Put your cursor after a `)` and see! Or set [rainbow parentheses](https://blog.rstudio.com/2020/11/04/rstudio-1-4-preview-rainbow-parentheses/)
...
* Combine data stored in separate data sets: e.g. Local lockdown measures and stationary bike sales.
