...
date: "April 21, 2022<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
So far we've been using dataframes to make visuals, manipulate and summarizing data. This has taken us pretty far!
...
We can produce them using the `:` regular sequence operator:
...
Character vectors store data as text and typically come up when dealing names, addresses, and ID Numbers:
...
```
Error in student_number + 10 : non-numeric argument to binary operator
```
---
# Factor Vectors

Factors are categorical data that encode a (modest) number of **levels**, like for sex, treatment group, or geographic region: 
